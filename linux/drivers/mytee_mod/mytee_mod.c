...
#include "../bluetooth/mytee_bt_client.h"
#include "variables.h"
...
#define MYTEE_IOCTL_VARIABLES_CONNECT_UP 0x15
#define MYTEE_IOCTL_VARIABLES_WRITE_UP 0x16
#define MYTEE_IOCTL_VARIABLES_CONNECT_DOWN 0x17
#define MYTEE_IOCTL_VARIABLES_WRITE_DOWN 0x18
...
{  
	switch(cmd)
	{
        ...
        case MYTEE_IOCTL_REQUEST_UNSHIELD_BT_CLIENT:
		{
			mytee_up_priv(MYTEE_UP_PRIV,0,0,0);
			u8* trusted_bt_client_write_flag = MYTEE_TRUSTED_BT_CLIENT_WRITE_FLAG_VIRT;
			if(*trusted_bt_client_write_flag==0x0){
                while(true){
                    if (curr_shield){
				    mytee_unshield_mmio(MYTEE_UNSHIELD_MMIO, curr_shield->skb, MYTEE_MMIO_CONTEXT_DEVICE_KEYBOARD, 0x1000);
					curr_shield = curr_shield->next;
                    }
                    else break;
                }
			}
		...
		case MYTEE_IOCTL_VARIABLES_CONNECT_UP:
		{
			connect_flag = true;
			break;
		}
		case MYTEE_IOCTL_VARIABLES_CONNECT_DOWN:
		{
			connect_flag = false;
			break;
		}
		case MYTEE_IOCTL_VARIABLES_WRITE_UP:
		{
			write_flag = true;
			break;
		}
		case MYTEE_IOCTL_VARIABLES_WRITE_DOWN:
		{
			write_flag = false;
			break;
		}
